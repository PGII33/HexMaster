...
class IASelector:
    ...
    def _select_hexarene_faction(self):
        ...
        max_units = min(24, ia_cp // 3)  # Limite par les cellules disponibles, minimum tier 3
        
        # Unités de tier 3-4 uniquement pour la faction du joueur
        units_tier3_4 = []
        for cls in unites.CLASSES_UNITES:
            tmp = cls("ennemi", (0,0))
            if tmp.faction == player_faction and tmp.tier >= 3:
        ...
        for cls in units_available:
            if cp_used + cls("ennemi", (0,0)).tier <= ia_cp and len(selected) < max_units:
                selected.append(cls)
    ...
    def _select_hexarene_faction_fallback(self, player_faction, ia_cp):
        ...
        units_faction.sort(key=lambda cls: cls("ennemi", (0,0)).tier, reverse=True)
    ...
    def _select_hexarene_libre(self):
        ...
        for cls in unites.CLASSES_UNITES:
            ...
            if tmp.tier >= 3:
                units_tier3_4.append(cls)
                if tmp.faction not in factions_dict:
                    factions_dict[tmp.faction] = []
                factions_dict[tmp.faction].append(cls)
        ...
        for faction in factions:
            ...
            faction_units.sort(key=lambda cls: cls("ennemi", (0,0)).tier, reverse=True)
    ...
    def _select_hexarene_libre_fallback(self, ia_cp):
        ...
        all_units = unites.CLASSES_UNITES[:]
        all_units.sort(key=lambda cls: cls("ennemi", (0,0)).tier, reverse=True)
