""" menu.py - Menu principal et gestion des états du jeu HexaMaster """
...
from sauvegarde import sauvegarder, creer_sauvegarde_defaut, charger, marquer_niveau_complete, appliquer_recompenses
...
class HexaMaster:
    def __init__(self):
        ...
        # Pour le VIDEORESIZE event
        self.is_fullscreen = True if self.display_mode == "fullscreen" else False
    ...
    def creer_boutons(self):
        ...
        self.boutons_menu = [
            Button((center_x-160, h//2-200, 320, 70), "Jouer",
                   self.open_playmenu, self.font_med),
            Button((center_x-160, h//2-120, 320, 70), "Inventaire",
                   self.open_inventaire, self.font_med),
            Button((center_x-160, h//2-40, 320, 70), "Boutique",
                   self.open_boutique, self.font_med),
            Button((center_x-160, h//2+40, 320, 70), "Custom",
                   self.open_custom_menu, self.font_med),
            Button((center_x-160, h//2+120, 320, 70), "Quitter",
                   lambda: sys.exit(), self.font_med),
        ]

        # Menu principal des options
        self.boutons_options_main = [
            Button((center_x-200, h//2-100, 400, 70), "Affichage",
                   self.open_option_display, self.font_med, BLEU),
            Button((center_x-200, h//2, 400, 70), "Sauvegarde",
                   self.open_option_save, self.font_med, BLEU),
            Button((center_x-200, h//2+100, 400, 70), "Retour",
                   self.retour_options, self.font_med, BLEU),
        ]
        ...
        self.boutons_options_save = [
            Button((center_x-200, h//2, 400, 70), "Réinitialiser Sauvegarde",
                   lambda: sauvegarder(creer_sauvegarde_defaut()), self.font_med, BLEU),
            Button((center_x-200, h//2+100, 400, 70), "Retour",
                   self.retour_options, self.font_med, BLEU),
        ]

        # Bouton Options en haut à droite - présent sur tous les écrans sauf dans les menus d'options
        self.bouton_option = Button(
            (w-140, 20, 120, 50), "Options", self.open_option, self.font_med)

        # Menu Jouer principal
        self.boutons_playmenu = [
            Button((center_x-140, h//2-100, 280, 50), "Campagne",
                   self.start_campagne, self.font_med),
            Button((center_x-140, h//2-40, 280, 50), "HexArène",
                   self.open_hexarene_menu, self.font_med),
            Button((center_x-140, h//2+20, 280, 50), "JcJ",
                   self.open_jcj_menu, self.font_med),
            Button((20, h-70, 150, 50), "Retour",
                   self.back_to_menu, self.font_med),
        ]

        # Sous-menu HexArène
        self.boutons_hexarene_menu = [
            Button((center_x-140, h//2-60, 280, 50), "Mode Faction",
                   self.start_hexarene_faction, self.font_med),
            Button((center_x-140, h//2, 280, 50), "Mode Libre",
                   self.start_hexarene_libre, self.font_med),
            Button((20, h-70, 150, 50), "Retour",
                   self.back_to_playmenu, self.font_med),
        ]

        # Sous-menu JcJ
        self.boutons_jcj_menu = [
            Button((center_x-140, h//2-60, 280, 50), "Local",
                   self.start_versus, self.font_med),
            Button((center_x-140, h//2, 280, 50), "En ligne",
                   self.start_en_ligne_placeholder, self.font_med),
            Button((20, h-70, 150, 50), "Retour",
                   self.back_to_playmenu, self.font_med),
        ]
        ...
        self.boutons_custom_menu = [
            Button((center_x-140, h//2-60, 280, 50), "Level Builder",
                   self.open_level_builder, self.font_med),
            Button((center_x-140, h//2, 280, 50), "Unite Builder",
                   self.open_unite_builder, self.font_med),
            Button((20, h-70, 150, 50), "Retour",
                   self.back_to_menu, self.font_med),
        ]
    ...
    def set_display_mode(self, mode):
        ...
        if mode == "windowed":
            self.screen = pygame.display.set_mode(
                (self.windowed_width, self.windowed_height), pygame.RESIZABLE)
    ...
    def start_campagne(self):
        ...
        if not os.path.exists(campaign_path):
            ...
            print(
                "Veuillez créer manuellement la structure de campagne ou ajouter des niveaux.")
            return
        ...
        self.jeu = Jeu(
            ...
            niveau_nom=config.nom if hasattr(
                config, 'nom') else f"Niveau {numero}"  # Utiliser le nom du niveau
        )
        self.etat = "jeu"
    ...
    def start_hexarene_faction(self):
        ...
        player_faction = player_units[0](
            "joueur", (0, 0)).get_faction() if player_units else "Morts-Vivants"
    ...
    def run(self):
        while True:
            ...
            if self.etat == "menu":
                ...
                self.screen.blit(
                    title, (self.screen.get_width()//2 - title.get_width()//2, 50))
                for b in self.boutons_options_main:
                    b.draw(self.screen)
                ...
                title = self.font_big.render(
                    "Options d'Affichage", True, (30, 30, 60))
                self.screen.blit(
                    title, (self.screen.get_width()//2 - title.get_width()//2, 50))
                for b in self.boutons_options_display:
                    b.draw(self.screen)
                ...
                self.screen.fill(BLANC)
                title = self.font_big.render(
                    "Options de Sauvegarde", True, (30, 30, 60))
                self.screen.blit(
                    title, (self.screen.get_width()//2 - title.get_width()//2, 50))
                for b in self.boutons_options_save:
                    b.draw(self.screen)
    ...
    def afficher_menu(self):
        ...
        self.screen.blit(title, (self.screen.get_width() //
    ...
    def afficher_placeholder(self):
        ...
        self.screen.blit(title, (self.screen.get_width() //
                         2 - title.get_width()//2, 250))
        if subtitle:
            self.screen.blit(subtitle, (self.screen.get_width() //
                             2 - subtitle.get_width()//2, 320))
        for b in self.boutons_retour_placeholder:
            b.draw(self.screen)
