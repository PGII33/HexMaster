...
class NiveauConfig:
    ...
    def __init__(self):
        ...
        # [(classe, position)]
        self.unites_imposees: List[Tuple[type, Tuple[int, int]]] = []
        self.placement_impose: bool = False  # Si True, les positions sont fixes
        # Liste des factions autorisées
        self.factions_autorisees: List[str] = []
        ...
        # [(classe, position)]
        self.unites_ennemis: List[Tuple[type, Tuple[int, int]]] = []
        ...
        # Autres récompenses éventuelles
        self.recompenses_autres: List[Dict[str, Any]] = []

        # Métadonnées
        # Si False, récompense une seule fois
        self.completable_plusieurs_fois: bool = False
    ...
    def valider(self) -> Tuple[bool, List[str]]:
        ...
        if self.type_restriction == TypeRestriction.UNITES_IMPOSEES:
            if not self.unites_imposees:
                erreurs.append(
                    "Des unités imposées doivent être définies pour ce type de restriction")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'NiveauConfig':
        ...
        config.unites_imposees = _convert_unit_names_to_classes(
            unites_imposees_data)

        config.placement_impose = data.get("placement_impose", False)
        config.factions_autorisees = data.get("factions_autorisees", [])
        config.faction_unique_requise = data.get(
        ...
        config.unites_ennemis = _convert_unit_names_to_classes(
            unites_ennemis_data)
        ...
        config.completable_plusieurs_fois = data.get(
